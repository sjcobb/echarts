...
class IntervalScale<SETTING extends ScaleSettingDefault = ScaleSettingDefault> extends Scale<SETTING> {
     ...
     */
    calcNiceTicks(splitNumber?: number, minInterval?: number, maxInterval?: number, binaryTicks?: boolean): void {
        ...
        const result = helper.intervalScaleNiceTicks(
            extent, span, splitNumber, minInterval, maxInterval, binaryTicks
        );
    ...
    calcNiceExtent(opt: {
        ...
        this.calcNiceTicks(opt.splitNumber, opt.minInterval, opt.maxInterval, opt.binaryTicks);
        const interval = this._interval;
